...
async function get(path: string) {
  ...
  const res = await fetch(`${host}/v1/${path}`, {
    headers: { Authorization: `Bearer ${token}`, "x-goog-user-project": getRagProjectId() },
  });
  console.log(path, res.status, (await res.text()).slice(0, 2500));
}

async function main() {
  loadEnv();
  const store = normalizeDataStoreResource(process.env.RAG_DATASTORE_PRODUCT ?? "");
  await get(`${store}/siteSearchEngine`);
  await get(`${store}/siteSearchEngine/targetSites`);
  await get(`projects/653828324568/locations/global/collections/default_collection/engines`);
}
