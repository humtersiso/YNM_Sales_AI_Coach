...
export function RoleplayStatsOverview({
  ...
  const kpis = [
    ...
    { label: "均分", value: hasData ? overallAvg : "—" },
    { label: "最近", value: lastScore != null ? lastScore : "—" },
  ];

  return (
    <section className="overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-sm">
      <div className="grid grid-cols-4 divide-x divide-emerald-100 bg-gradient-to-b from-teal-50/50 to-white px-1 py-3">
        {kpis.map((k) => (
          <div key={k.label} className="min-w-0 px-1 text-center">
            <p className="text-xs font-medium text-emerald-600">{k.label}</p>
            <p className="mt-1.5 text-lg font-semibold tabular-nums text-emerald-950">{k.value}</p>
            {k.hint ? (
      ...
      {hasData && dimensions ? (
        <div className="border-t border-emerald-50 px-2 pb-2 pt-1.5">
          <RoleplayRadarChart
            variant="overview"
        ...
        <p className="border-t border-emerald-50 px-4 py-6 text-center text-sm text-emerald-600">
          完成首場對練後顯示五維雷達
        </p>
      )}
    </section>
