...
### 4.2 部分開發（裕日資料庫與 BigQuery 優先；資料整理 Agent 平台 → 裕日銷售知識庫 → 以 ADK 實作兩個 Agent 串知識庫；測試範疇見下段說明）

本方案同樣以 **裕日資料庫** 為主資料核心，**先穩定寫入 BigQuery**，再建置 **總部資料整理 Agent 平台**、**裕日銷售知識庫**，最後以 **ADK（Agent Development Kit，Google 提供之智慧代理開發工具組）** 實作 **銷售助手、對練助手** 兩個 Agent 與知識庫串接。**不含**完整第一線產品化畫面與完整管理後台（4a～4d）。
